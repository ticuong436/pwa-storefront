...
const mediaBases = new Map()
    ...
    .set('image-feature', 'feature_banner/')
    .set('image-banner', 'banner_image/')
    .set('image-swatch', 'attribute/swatch/')
    .set('image-updateinfo', 'updateinfo_image/');
...
const makeAssetUrl = path => {
    if (!path) {
        return null;
    }
    const baseUrl = process.env.PUBLIC_PATH;

    return joinUrls(baseUrl, path);
};

export const isInHome = path => joinUrls(path, '') === makeAssetUrl('/');

const makeMediaUrl = (path, type, opts) => {
    if (!path) {
        return null;
    }
...
const makeOptimizedUrl = (path, { type, ...opts } = {}) => {
    ...
    if (!type || !type.startsWith('image-')) {
        return makeAssetUrl(path);
    }

    return makeMediaUrl(path, type, opts);
};
